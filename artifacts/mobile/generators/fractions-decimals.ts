...
export function genFractions(cls: SchoolClass, diff: Difficulty): Question {
  type TQ = () => Question;

  // Class 3: ½, ¼, ⅓ of amounts — simple, picture-able fractions
  if (cls === '3rd') {
    const class3: TQ[] = [
      () => { const whole = pick([2, 4, 6, 8, 10, 12]); return { questionText: `½ of ${whole} = ?`, answer: whole / 2, choices: makeIntChoices(whole / 2) }; },
      () => { const whole = pick([4, 8, 12, 16, 20]); return { questionText: `¼ of ${whole} = ?`, answer: whole / 4, choices: makeIntChoices(whole / 4) }; },
      () => { const whole = pick([3, 6, 9, 12, 15]); return { questionText: `⅓ of ${whole} = ?`, answer: whole / 3, choices: makeIntChoices(whole / 3) }; },
      () => { const whole = pick([4, 8, 12, 16, 20]); return { questionText: `¾ of ${whole} = ?`, answer: (whole / 4) * 3, choices: makeIntChoices((whole / 4) * 3) }; },
  ...
  if (cls === '4th') {
    const easy: TQ[] = [
      () => { const d = pick([2, 4, 8]); const n = ri(1, d - 1); const whole = d * ri(2, 10); return { questionText: `${n}/${d} of ${whole} = ?`, answer: n * whole / d, choices: makeIntChoices(n * whole / d) }; },
      () => { const d = pick([2, 3, 4]); return { questionText: `Complete: 1/${d} + 1/${d} = ?/${d}`, answer: 2, choices: makeIntChoices(2) }; },
      () => { const n = ri(2, 4); const d = n * 2; return { questionText: `Simplify ${n}/${d} = 1/?`, answer: d / n, choices: makeIntChoices(d / n) }; },
    ];
    const medium: TQ[] = [
      () => { const d = pick([3, 4, 5, 6]); const n = ri(2, d - 1); const whole = d * ri(2, 8); return { questionText: `${n}/${d} of ${whole} = ?`, answer: n * whole / d, choices: makeIntChoices(n * whole / d) }; },
  ...
  const medium: TQ[] = [
    () => { const d = pick([3, 4, 5, 6]); const n = ri(2, d - 1); const whole = d * ri(2, 8); return { questionText: `${n}/${d} of ${whole} = ?`, answer: n * whole / d, choices: makeIntChoices(n * whole / d) }; },
    () => { const d = pick([4, 6, 8, 10]); const a = ri(1, Math.floor(d / 2)); const b = ri(1, Math.floor(d / 2) - a + 1); return { questionText: `${a}/${d} + ${b}/${d} = ?/${d}`, answer: a + b, choices: makeIntChoices(a + b) }; },
    () => { const g = ri(2, 4); const a = g * ri(2, 4); const b = g * ri(2, 4); return { questionText: `What is the HCF (simplify):\n${a}/${b}? (HCF = ?)`, answer: g, choices: makeIntChoices(g) }; },
  ];
  const hard: TQ[] = [
    () => { const d = pick([6, 8, 10, 12]); const a = ri(3, d - 2); const b = ri(1, 3); return { questionText: `${a}/${d} − ${b}/${d} = ?/${d}`, answer: a - b, choices: makeIntChoices(a - b) }; },
    () => { const d1 = pick([2, 3, 4]); const d2 = d1 * 2; return { questionText: `1/${d1} + 1/${d2} = ?/${d2}`, answer: 3, choices: makeIntChoices(3) }; },
    () => { const n = ri(2, 5); const d = ri(2, 4); const whole = d * ri(3, 8); return { questionText: `What is ${n} whole and 1/${d} of ${whole}?\n(${n} × ${whole}) + (${whole}/${d}) = ?`, answer: n * whole + whole / d, choices: makeIntChoices(n * whole + whole / d) }; },
  ];
  return pick(diff === 'easy' ? easy : diff === 'medium' ? medium : hard)();
}

export function genDecimals(cls: SchoolClass, diff: Difficulty): Question {
  ...
  const easy: TQ[] = [
    ...
    () => { const n = ri(11, 39); return { questionText: `Round ${n / 10} to the nearest whole number`, answer: Math.round(n / 10), choices: makeIntChoices(Math.round(n / 10)) }; },
  ];
  const medium: TQ[] = [
  ...
  const hard: TQ[] = [
    ...
    () => { const n = ri(105, 299); return { questionText: `Round ${n / 100} to 1 decimal place`, answer: round(Math.round(n / 10) / 10), choices: makeDecChoices(round(Math.round(n / 10) / 10), 0.1) }; },
