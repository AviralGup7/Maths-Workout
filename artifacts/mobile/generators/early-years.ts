...
export function genCounting(cls: SchoolClass, diff: Difficulty): Question {
  if (diff === 'hard') {
...
export function genNumberSense(cls: SchoolClass, diff: Difficulty): Question {
  ...
  if (diff === 'hard') {
    ...
    if (isEarly) {
      ...
      return {
        questionText: `Put in order — SMALLEST first:\n${arr.join('  ')}`,
        answer: sorted[0],
    ...
    const sorted = [...arr].sort((a, b) => a - b);
    return {
      questionText: `Put in order. What is the SMALLEST?\n${arr.join('  ')}`,
      answer: sorted[0],
      choices: makeIntChoices(sorted[0]),
    };
  }
  ...
  if (isEarly) {
    const templates = [
      () => {
        ...
        return { questionText: `Which is bigger?\n${a}   or   ${b}`, answer: bigger, choices: makeIntChoices(bigger) };
      },
      () => { const n = ri(0, max - 1); return { questionText: `What comes after ${n}?`, answer: n + 1, choices: makeIntChoices(n + 1) }; },
      () => { const n = ri(1, max); return { questionText: `What is 1 more than ${n}?`, answer: n + 1, choices: makeIntChoices(n + 1) }; },
      () => { const n = ri(2, max); return { questionText: `What comes before ${n}?`, answer: n - 1, choices: makeIntChoices(n - 1) }; },
    ];
  ...
  const templates = [
    () => { const a = ri(1, max); let b = ri(1, max); if (a === b) b = b < max ? b + 1 : b - 1; const bigger = Math.max(a, b); return { questionText: `Which is bigger?\n${a}   or   ${b}`, answer: bigger, choices: makeIntChoices(bigger) }; },
    () => { const n = ri(0, max - 1); return { questionText: `What comes after ${n}?`, answer: n + 1, choices: makeIntChoices(n + 1) }; },
    () => { const n = ri(1, max); return { questionText: `What is 1 more than ${n}?`, answer: n + 1, choices: makeIntChoices(n + 1) }; },
    () => { const n = ri(10, max); return { questionText: `What is 10 less than ${n}?`, answer: n - 10, choices: makeIntChoices(n - 10) }; },
    () => { const n = ri(2, max); return { questionText: `What comes before ${n}?`, answer: n - 1, choices: makeIntChoices(n - 1) }; },
  ];
